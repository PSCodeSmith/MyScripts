...
[CmdletBinding()]
param (
    [Parameter(Mandatory = $false, HelpMessage = "Run only during work hours?")]
    [bool]$RunDuringWorkHours = $false,

    [Parameter(Mandatory = $false, HelpMessage = "Start day of the workweek.")]
    [ValidateSet('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday')]
    [string]$StartDayOfWeek = 'Monday',

    [Parameter(Mandatory = $false, HelpMessage = "End day of the workweek.")]
    [ValidateSet('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday')]
    [string]$EndDayOfWeek = 'Friday',

    [Parameter(Mandatory = $false, HelpMessage = "Start time of the workday (HH:mm:ss).")]
    [string]$StartTimeOfDay = '07:30:00',

    [Parameter(Mandatory = $false, HelpMessage = "End time of the workday (HH:mm:ss).")]
...
function Keep-Active {
    ...
    while ($true) {
        ...
        if ($currentTime -ge $nextCheckTime) {
            ...
            if ($batteryStatus -ne 2) { # Not plugged in
                Start-Sleep -Seconds 1800
                $nextCheckTime = $currentTime.AddSeconds(1800)
                continue
            }
